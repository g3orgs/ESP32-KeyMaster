...
String getHtmlContent() {
  String html = FPSTR(CONFIG_PAGE_HEAD);
  html.reserve(3072);
...
void handleSave() {
  ...
  if (server.hasArg("keyA") && server.hasArg("rA") && server.hasArg("gA") && server.hasArg("bA") &&
    ...
    String html = FPSTR(SAVE_PAGE_HEAD);
    html += "<form action='/exit' method='POST' style='margin:0;'>";
    html += "<button type='submit' class='secondary' style='padding:12px 24px;border:0;border-radius:5px;font-weight:bold;cursor:pointer;'>Exit & Restart Device</button>";
    html += "</form>";
    html += FPSTR(SAVE_PAGE_FOOT);
...
void handleDebug() {
  ...
  String html = FPSTR(DEBUG_PAGE_HEAD);
...
void handleHelp() {
  String html = FPSTR(HELP_PAGE_HEAD);
  ...
  html += "</table>";
  html += FPSTR(HELP_PAGE_FOOT);
  server.send(200, "text/html", html);
}

// Neue About-Seite Handler
void handleAbout() {
  String html = FPSTR(ABOUT_PAGE_HEAD);
  html += FW_VERSION;
  html += FPSTR(ABOUT_PAGE_MID);
...
void setupWebServerRoutes() {
  ...
  server.on("/exit", HTTP_GET, handleExit);  // Allow link-based exit
